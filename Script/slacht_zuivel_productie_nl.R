...
ggplot(slacht_complete_month, aes(x = Perioden_Date, y = AantalSlachtingen_1)) +
  ...
  theme_bw() + 
  scale_x_date(breaks = "10 year", minor_breaks = "1 year", date_labels =  "%Y") + 
  scale_y_continuous(labels = comma) +
  facet_wrap(.~Slachtdieren_label, scales = "free") +
  labs(title = "slachtingen per maand vanaf 1990", x = "Jaar" , y = "slachtingen x1000")
dev.copy(png,'output/slachtingen_vanaf_1990.png',width=1200, height=720)
dev.off()
...
dev.copy(png,'output/zuivelproductie_vanaf_1995.png',width=1200, height=720)
dev.off()
